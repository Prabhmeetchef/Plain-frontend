...
export default function Avatars() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (type === "html_css") {
      ...
      newAvatar = { type, title, html, css, notes };
    } else if (type === "image") {
      const imgurl = formData.get("imgurl") as string;
      newAvatar = { type, title, imgurl, notes };
    } else {
      return;
    }
    ...
    const updatedAvatars = [...(userData?.avatars || []), newAvatar];
    ...
    (document.getElementById("add-avatar-dialog") as HTMLDialogElement).close();
    router.refresh();
  }
  ...
  return (
    <div className="h-[100vh] flex justify-between items-center">
      ...
      <div className="flex-grow flex-col h-full py-6 pr-1">
        <h1 className="text-[32px] font-semibold">Avatars</h1>
        <h2 className="opacity-60">
          All avatar components can be seen and created here.
        </h2>

        <div className="flex flex-wrap gap-8 py-10">
          <div className="flex items-center justify-center w-[300px] h-[300px]">
            {/* Add min-width to dialog */}
            <dialog id="add-avatar-dialog" className="rounded-lg p-6 w-[500px] min-w-[500px]">
            ...
            <button
              ...
              onClick={() =>
                (document.getElementById("add-avatar-dialog") as HTMLDialogElement).showModal()
              }
            >
              <Image
                src="/+.png"
          ...
          {avatars.map((avatar, index) => (
            <div key={index}>
              <dialog id={`avatar-dialog-${index}`} className="rounded-lg p-6 w-[500px] min-w-[500px]">
                <h1 className="mb-[10px] text-[#AB00D6] w-[62px] font-semibold text-[20px]">{avatar.title}</h1>
                ...
                <button
                  type="button"
                  onClick={() =>
                    (document.getElementById(`avatar-dialog-${index}`) as HTMLDialogElement).close()
                  }
                  className="flex py-2 px-[20px] rounded-[6px] bg-[#E6E6E6]"
                >
                  Close
              ...
              <div
                ...
                onClick={() =>
                  (document.getElementById(`avatar-dialog-${index}`) as HTMLDialogElement).showModal()
                }
              >
                {avatar.type === "html_css" ? (
                  <>
                  ...
                  <>
                    <div className="flex w-full bg-[#e9e9e9] rounded-[6px] items-center justify-center h-full">
                      <Image
                        src={avatar.imgurl || "/default-image.png"}
                        alt={avatar.title}
                        width={200}
                        height={100}
