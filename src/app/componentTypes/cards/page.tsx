...
export default function Cards() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (type === "html_css") {
      ...
      newCard = { type, title, html, css, notes };
    } else if (type === "image") {
      const imgurl = formData.get("imgurl") as string;
      newCard = { type, title, imgurl, notes };
    } else {
      return;
    }
    ...
    (document.getElementById("add-card-dialog") as HTMLDialogElement).close();
    router.refresh();
  }
  ...
  return (
    <div className="h-[100vh] flex justify-between items-center">
      ...
      <div className="flex-grow flex-col h-full py-6 pr-1">
        <h1 className="text-[32px] font-semibold">Cards</h1>
        <h2 className="opacity-60">
          All card components can be seen and created here.
        </h2>

        <div className="flex flex-wrap gap-8 py-10">
          <div className="flex items-center justify-center w-[300px] h-[300px]">
            {/* Add min-width to dialog */}
            <dialog id="add-card-dialog" className="rounded-lg p-6 w-[500px] min-w-[500px]">
            ...
            <button
              ...
              onClick={() =>
                (document.getElementById("add-card-dialog") as HTMLDialogElement).showModal()
              }
            >
              <Image
                src="/+.png"
          ...
          {cards.map((card, index) => (
            <div key={index}>
              <dialog id={`card-dialog-${index}`} className="rounded-lg p-6 w-[500px] min-w-[500px]">
                <h1 className="mb-[10px] text-[#AB00D6] w-[62px] font-semibold text-[20px]">{card.title}</h1>
                {card.type === "html_css" ? (
                  <>
                    <div className="py-4">
                      <h1 className="opacity-60 text-[13px]">HTML</h1>
                ...
                <button
                  type="button"
                  onClick={() =>
                    (document.getElementById(`card-dialog-${index}`) as HTMLDialogElement).close()
                  }
                  className="flex py-2 px-[20px] rounded-[6px] bg-[#E6E6E6]"
                >
                  Close
                    ...
                    <div className="flex-1 flex w-full overflow-auto bg-[#e9e9e9] rounded-[6px] items-center justify-center">
                      ...
                      <iframe
                        srcDoc={`
                          ...
                          <html>
                            <head>
                              <style>${card.css || ""}body {display: flex; align-items: center; justify-content: center; height: 92vh;}</style>
                            </head>
                            <body>${card.html || ""}</body>
                          </html>
                        `}
