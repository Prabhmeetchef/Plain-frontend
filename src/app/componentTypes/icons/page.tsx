...
export default function Icons() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (type === "html_css") {
      ...
      newIcon = { type, title, html, css, notes };
    } else if (type === "image") {
      const imgurl = formData.get("imgurl") as string;
      newIcon = { type, title, imgurl, notes };
    } else {
      return;
    }
    ...
    const updatedIcons = [...(userData?.icons || []), newIcon];
    const { error: updateError } = await supabase
      .from("users")
      .update({ icons: updatedIcons })
      .eq("email", session?.user?.email);
    if (updateError) {
    ...
    (document.getElementById("add-icon-dialog") as HTMLDialogElement).close();
    router.refresh();
  }
  return (
    <div className="h-[100vh] flex justify-between items-center">
      ...
      <div className="flex-grow flex-col h-full py-6 pr-1">
        <h1 className="text-[32px] font-semibold">Icons</h1>
        <h2 className="opacity-60">
          All icon components can be seen and created here.
        </h2>
          ...
          <div className="flex items-center justify-center w-[300px] h-[300px]">
            ...
            >
              <form onSubmit={handleSubmit}>
                ...
                <Tabs defaultValue="html_css" className="w-full">
                  {" "}
                  {/* Set default value to html_css */}
                ...
                {icon.type === "html_css" ? (
                        ...
                        srcDoc={`
                          ...
                          <html>
                            <head>
                              <style>${
                                icon.css || ""
                              }body {display: flex; align-items: center; justify-content: center; height: 92vh;}</style>
                            </head>
                            <body>${icon.html || ""}</body>
                          </html>
                        `}
                  ...
                  <>
                    <div className="flex w-full bg-[#e9e9e9] rounded-[6px] items-center justify-center h-full">
                      <Image
                        src={icon.imgurl || "/default-image.png"}
                        alt={icon.title}
                        width={200}
                        height={100}
