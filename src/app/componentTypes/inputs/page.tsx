...
export default function Inputs() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (type === "html_css") {
      ...
      newInput = { type, title, html, css, notes };
    } else if (type === "image") {
      const imgurl = formData.get("imgurl") as string;
      newInput = { type, title, imgurl, notes };
    } else {
      return;
    }
    ...
    const updatedInputs = [...(userData?.inputs || []), newInput];
    ...
    (document.getElementById("add-input-dialog") as HTMLDialogElement).close();
    router.refresh();
  }
  return (
    <div className="h-[100vh] flex justify-between items-center">
      ...
      <div className="flex-grow flex-col h-full py-6 pr-1">
        <h1 className="text-[32px] font-semibold">Inputs</h1>
        <h2 className="opacity-60">
          All input components can be seen and created here.
        </h2>
                ...
                {input.type === "html_css" ? (
                  ...
                  <>
                    <div className="flex w-full bg-[#e9e9e9] rounded-[6px] items-center justify-center h-full">
                      <Image
                        src={input.imgurl || "/default-image.png"}
                        alt={input.title}
                        width={200}
                        height={100}
